...
/**
 * Internal helper: builds a jsPDF document for a session chart and returns it.
 */
async function buildSessionPdfDoc(options: PdfChartOptions): Promise<JsPDFType> {
...
/**
 * Builds a session-ready PDF chart and returns the raw bytes without triggering
 * a browser download. Safe to call in Node.js/server-side contexts.
 */
export async function buildSessionPdfBytes(options: PdfChartOptions): Promise<Uint8Array> {
  const doc = await buildSessionPdfDoc(options);
