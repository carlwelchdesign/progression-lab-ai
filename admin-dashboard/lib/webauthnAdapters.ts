...
export const webAuthnConfigProvider: WebAuthnConfigProvider = {
  getConfig() {
    return {
      ...
      // Admin has its own origin env var with fallback to shared one
      expectedOrigin:
...
export const webAuthnCredentialStore: CredentialStore = {
  ...
  async findActiveCredentialForUser(userId, credentialId) {
    const credential = await prisma.webAuthnCredential.findFirst({
      where: { credentialId, userId, revokedAt: null },
    });
    return credential ? toStoredCredential(credential) : null;
  },
  ...
  async upsertCredential(input: UpsertCredentialInput) {
    ...
    return toStoredCredential(credential);
  },
